...
class InputTools():
    ...
    class MkInputFile():
        ...
        def into_package() -> None:
            ...
            value_path = SimulationModelKey.MARTENSITE.PATH.VALUE
            model_path = SimulationModelKey.MARTENSITE.PATH.MODEL
            for model in [SimulationModelKey.MARTENSITE.TOWARD_STABLE]:
                input_file_fmt = InputFileFmt(model=model)
                stamp_format = StampFormat(model=model)
                ...
                format_dir = os.path.join(SimulationModelKey.MARTENSITE.PATH.DATA, model[SimulationModelKey.CLASS_NAME])
                if os.path.exists(format_dir) == False:
                    os.mkdir(format_dir)
                fmt_path = os.path.join(format_dir, "format.yaml")
